...
pub fn make_outbound_action(
	...
	match rule.kind.as_str() {
		"socks5" => Arc::new(SocksOutbound::new(SocksOutboundOpts {
			...
			stream_timeout,
			tcp_keepalive: Some(get_tcp_keepalive()),
		})),
...
/// Get TCP keepalive configuration to prevent FIN_WAIT2 socket leak.
///
/// This configuration aggressively probes idle connections to ensure
/// they are properly closed when the remote peer is unresponsive.
fn get_tcp_keepalive() -> wind_core::tcp::TcpKeepalive {
	wind_core::tcp::TcpKeepalive {
		enable: true,
		idle: Duration::from_secs(30),      // Wait 30s before first keepalive probe
		interval: Duration::from_secs(3),   // Send probe every 3s after idle timeout
		count: 5,                            // Close connection after 5 failed probes (15s)
	}
}
